...
| 执行评测（可选） | 量化本次交付质量 | 6维评分报告（流程完整性/产物质量/实现正确性/安全合规/效率/迭代能力） |
...
`delivery-loop` 内置了一个可选的执行评测步骤（Step 5.5），用 **6 维量化评分** 回答"这次做得到底好不好"。评测是完全确定性的——检查产物文件是否存在、编译是否通过、测试是否跑过，零 LLM 评委参与，同一批 case 跑 3 次分数 hash 完全一致。

### 何时触发评测

| 时机 | 适用场景 | 产出 |
|------|----------|------|
| Step 5 质量门通过后 | 日常开发，快速反馈 | 评分简报，辅助验收判断 |
| Step 8 结项沉淀时 | 完整复盘，基线对比 | 详细评分报告，进 memory |

### 6 个评分维度

| 维度 | 权重 | 检查什么 |
|------|------|----------|
| 流程完整性 | 25% | 必走阶段的产物文件是否都存在（不靠 AI 说做了，靠文件系统验证） |
| 产物质量 | 25% | requirements/plan 是否有实质内容而非模板套话 |
| 实现正确性 | 25% | **真跑编译 + 真跑测试**，不信自报，检测 honest gap |
| 安全合规 | 10% | 是否跳过质量门、配置问题是否在验收时才暴露 |
| 效率 | 10% | 耗时 + 工具调用次数，与基线对比 |
| 迭代能力 | 5% | 失败后能否自愈（BUILD FAILURE → SUCCESS 恢复链条） |

综合等级：**S (≥90) / A (≥80) / B (≥70) / C (≥60) / D (<60)**。
...
```text
$delivery-loop
需求已发布观察完成。请复盘本次交付，对整轮执行做 6 维评测，并把评分报告写入 memory。
```

**方式三：拿评分驱动流程改进**

```text
当前版本 → 评测（6 维）→ 发现弱项 → 修改流程/规则 → 再次评测对比分数
```

你可以让 AI 拿着"高分配置"去改造"低分配置"，用确定性分数验证优化是否有效。评测模板位于 [`references/eval-template.md`](references/eval-template.md)。
...
- 评测分数是流程改进的硬证据：每次修改流程/规则后，用评测分数验证是变好还是变坏，而非凭感觉。
...
```text
$delivery-loop
请对本轮交付的产物文件、测试结果和流程完整性做 6 维量化评分，输出评分报告并标明等级。
```
